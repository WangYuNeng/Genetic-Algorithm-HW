...
void GA::initializePopulation ()
{
    ...
    // also initialize fitness to calculate selection intensity
    stFitness.reset ();
...
void GA::populationwiseShuffle () 
{
    for ( int i = 0; i < ell; ++i ) {
        if (myRand.uniform () < pc) {
            int *newIdx = new int[nNextGeneration];
            myRand.uniformArray(newIdx, nNextGeneration, 0, nCurrent-1);
            for ( int j = 0; j < nNextGeneration; ++j ) {
                offspring[j].setVal(i, population[selectionIndex[newIdx[j]]].getVal(i));
            }
            delete [] newIdx;
        }
        else {
            for ( int j = 0; j < nNextGeneration; ++j ) {
                offspring[j].setVal(i, population[selectionIndex[j]].getVal(i));
            }
        }
...
void GA::showStatistics ()
{
    ...
    printf ("Gen:%d  Fitness:(Max/Mean/Min/Std):%f/%f/%f/%f Chromsome Length:%d\n",
        generation, stFitness.getMax (), stFitness.getMean (),
        stFitness.getMin (), stFitness.getStdev(), population[0].getLength ());
...
void GA::oneRun (bool output)
{
    ...
    double currentFitness = stFitness.getMean();
    double currentStd = stFitness.getStdev();
    ...
    selectionIntensities.push_back( ( stFitness.getMean() - currentFitness ) / currentStd );
...
int GA::doIt (bool output)
{
    ...
    printf("%d", generation);
    for ( int i = 0; i < selectionIntensities.size(); ++i ) {
        printf(",%lf", selectionIntensities[i]);
    }
    printf("\n");
    return generation;
}
